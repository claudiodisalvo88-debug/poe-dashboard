# RQ-026 — Innovator structural property directions
...
* This file is research-framing only.
* A/B/D are directions only.
...
Directional meaning:
Direction removed from active `RQ-026`.
Critic status: SPLIT REQUIRED.
Candidate future `RQ-027` wording:
...
Constraint:
This direction must not advance inside `RQ-026`.
It is not accepted as active `RQ-026` direction.
It is recorded only as future `RQ-027` candidate and does not open `RQ-027`.
