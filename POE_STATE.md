# POE_STATE.md

Active milestone:
M-002

M-001:
CLOSED / COMPLETED

M-002:
OPENED / RESEARCH-ONLY

Active research:
RQ-029

Last closed research:
RQ-025

Last postponed research:
RQ-009

RQ-005B:
VALIDATED

RQ-007:
VALIDATED / CLOSED

RQ-008:
VALIDATED / CLOSED

RQ-009:
POSTPONED

RQ-010:
VALIDATED / CLOSED

RQ-011:
VALIDATED / CLOSED

RQ-012:
VALIDATED / CLOSED

RQ-013:
VALIDATED / CLOSED

RQ-014:
VALIDATED / CLOSED

RQ-015:
VALIDATED / CLOSED

RQ-016:
VALIDATED / CLOSED

RQ-017:
VALIDATED / CLOSED

RQ-018:
VALIDATED / CLOSED

RQ-019:
VALIDATED / CLOSED

RQ-020:
VALIDATED / CLOSED

RQ-021:
VALIDATED / CLOSED

RQ-022:
VALIDATED / CLOSED

RQ-023:
VALIDATED / CLOSED

RQ-024:
CONDITIONAL / CLOSED

RQ-025:
VALIDATED / CLOSED

RQ-026:
OPEN / RESEARCH-FRAMING

RQ-027:
OPEN / RESEARCH-FRAMING

RQ-029:
OPEN / RESEARCH-FRAMING

RQ-028:
OPEN / RESEARCH-FRAMING

PD-000 to PD-023:
VALIDATED

PD-024:
CONDITIONAL

PD-025:
VALIDATED

Formula candidates:
RESEARCH PHASE OPENED / NO CANDIDATES DEFINED

Final formula:
NOT DEFINED

Implementation:
BLOCKED

Operating workflow:
Repository-based agent workflow formalized through AGENTS.md, WORKFLOW_RUNBOOK.md, AGENT_QUEUE.md, AGENT_OUTPUTS.md and PM_REVIEW.md.
Protocol Kernel consolidation completed through POE_PROTOCOL_KERNEL.md and README.md update.
Protocol Kernel milestone M-001 closed through M-001_PROTOCOL_KERNEL_MILESTONE_CLOSURE.md.
Formula candidate research phase M-002 opened through M-002_FORMULA_CANDIDATE_RESEARCH_PHASE_OPENING.md.

Protocol rule from RQ-007:
CONTRIBUTO, RIDUZIONE and VALORE are distinct objects.
PD-001 is a protocol hierarchy, not a rigid causal order.
Observed behavior is only candidate contribution until linked to verified reduction, attribution and systemic relevance.

Next action:
Documentation-readiness review for `RQ-029` research-framing only, while keeping `RQ-029` OPEN / RESEARCH-FRAMING with Decision `NONE`; keeping `RQ-026`, `RQ-027` and `RQ-028` OPEN / RESEARCH-FRAMING with Decision `NONE`; keeping `PD-026`, `PD-027`, `PD-028` and `PD-029` absent; keeping formula candidates as `RESEARCH PHASE OPENED / NO CANDIDATES DEFINED`; keeping final formula `NOT DEFINED`; keeping implementation blocked.
